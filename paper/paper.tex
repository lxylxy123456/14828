...
In this section, we explain how we reproduce the attack. We provide a fully functioning environment for demonstrating this attack. We write two websites using Flask as the attacker and the victim website. We also add CSP logics to demonstrate the recommended mitigation in \cite{abp_code_injection}.
...
We implement the attacker website using Flask as the backend. We use plain HTML and JavaScript as the frontend. The attacker website has two features.
...
Similar to the attacker website, we implement the attacker website using Flask as the backend. We use plain HTML and JavaScript as the frontend. The victim website has multiple sections, as discussed below in detail.
